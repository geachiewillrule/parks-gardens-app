...
const App = () => {
  ...
  const [loginForm, setLoginForm] = useState({ email: 'john.smith@cityparks.gov', password: 'password' });
  ...
  const startTaskWithSafety = (taskId: number) => {
    ...
    // If task has no safety documents, start immediately
    if (!task?.riskAssessment && !task?.swms) {
  ...
  const confirmSafetyAndStart = async () => {
    ...
    // Check if safety docs are acknowledged (if they exist)
    const raOk = !task?.riskAssessment || raAcknowledged;
    const swmsOk = !task?.swms || swmsAcknowledged;
    
    if (raOk && swmsOk && currentTaskId) {
